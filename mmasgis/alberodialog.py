...
from selezioneGeo import SelezioneGeoWindow
from cercaGeo import MainWindowCercaGeo
...
class MainWindowAlbero(QMainWindow, Ui_Albero):
	...
	"""
	@return: [Nodo] lista delle Utb presenti nella finestra delle Utb selezionate
	"""
	def getList(self):
		print "%%%lista in alberoDialog "+ str( len( self.selectionGui.getList()))
		print self.selectionGui.getList()
		#settrace()
		return self.selectionGui.getList()
	...
	def on_cercaButton_released(self):
		...
		self.cerca.move((screen.width()/2)-size.width()/2,(screen.height()/2)-size.height())
		self.cerca.show()
...
#		settrace()
	...
	def selectNode(self,n):
		...
		print "elementi in selectionGui "+str(len(self.selectionGui.getList()))
		self.selectionGui.updateList()
