...
class User():
	...
	def setActiveDb(self,db):
		...
		for i in self.registeredDbs:
			i.setActive(False)
			# setto attivo il db il cui id corrisponde a quello passato
		for i in self.registeredDbs:
			if i.getId()==Id:
				i.setActive(True)
				self.activeDb=i 
...
class Db():
	...
	def getConnectionString(self):
		...
		il formato dela stringa e'
		user:pwd@host/db
		@return: string
		"""
		"root:vilu7240@localhost/parafarmacie"
		str="{0}:{1}@{2}:{3}/{4}".format(self.user_name,self.password,self.host,self.port,self.censimento)
		return str	
	
	def getConnectionStringMMasgisDB(self):
		"""ritorna la stringa di connessione per sqlalchemy
		il formato dela stringa e'
		user:pwd@host/db
		@return: string
		"""
		"root:vilu7240@localhost/parafarmacie"
		str="{0}:{1}@{2}:{3}/{4}".format(self.user_name,self.password,self.host,self.port,"mmasgisDB")
		return str	
	def mysqlConnect(self):
		return self.cons.dbDrivers[str(self.getRDBMS())].connect(host=self.getHost(), user=self.getUserName(), passwd=self.getPassword(),db=self.getNameDb(),charset="utf8",use_unicode=True)



	def mysqlConnect2Metmi(self):
		return self.cons.dbDrivers[str(self.getRDBMS())].connect(host=self.getHost(), user="metmi", passwd='metmi',db='mmasgisDB',charset="utf8",use_unicode=True)
	...
	def pgConnect(self):
		...
		conn= self.cons.dbDrivers[str(self.getRDBMS())].connect("host='{0}' user='{1}' password='{2}' dbname='{3}'".format(self.getHost(),self.getUserName(),self.getPassword(),self.getNameDb()))
		return conn
	
	def pgConnect2Metmi(self):
		#print "pg connection string","host='{0}' user='{1}' password='{2}' dbname='{3}'".format(self.getHost(),self.getUserName(),self.getPassword(),self.getNameDb())
		conn= self.cons.dbDrivers[str(self.getRDBMS())].connect("host='{0}' user='{1}' password='{2}' dbname='{3}'".format(self.getHost(),'metmi','metmi','mmasgisDB'))
		return conn
	...
	def __init__(self,dbName,Id,descrizione="",user_name="",pwd="",host="",rdbms=""):
		...
		@param string:descrizione opzionale
		@param string:user name opzionale
		@param string: passwordopzionale
		@param string: host opzionale
		@param string: famiglia db<mysql><postgres> ... opzionale     
		"""
		self.certificato=-1
		self.pv_mt=-1
		self.cons=cons()
	...
	def getPv_mt(self):
		"""
		ritorna il parametro pv_mt, che indica  se occorre fare query su anagrafiche certificate
		""" 
		return self.pv_mt  
	...
	def getConnection(self):
		...
		connectFunctions['postgresql']=self.pgConnect	
		return connectFunctions[self.rdbms]()	
	
	def getConnectionMMASGISDB(self):
		connectFunctions={}
		connectFunctions['mysql']=self.mysqlConnect2Metmi
		connectFunctions['postgresql']=self.pgConnect2Metmi
		#print "rdbms in sessionClasses:",self.rdbms
		return connectFunctions[str(self.rdbms)]()
	
	def getConnection2Metmi(self):
		connectFunctions={}
		connectFunctions['mysql']=self.mysqlConnect2Metmi
		connectFunctions['postgres']=self.pgConnect2Metmi
		#print "rdbms in sessionClasses:",self.rdbms
		return connectFunctions[str(self.rdbms)]()	
	...
	def __repr__(self):
		...
		return "DB "+l(self.active)+" : rdbms:{0}, nome:{1}, descrizione:{2}, host:{3},id:{4}, port: {5}".format(self.rdbms,self.name,self.descrizione,self.host,self.Id,self.port)

class Profile_standard(Profile):
	def __init__(self,role={}):
		Profile.__init__(self, role)
		self.name=""
		self.company=None
		self.Id=-1
	...
	def isAdministrator(self):
		return type(self.profile)==type(1)
		
	def getMapper(self,session):
		
		"""produce gli oggetti sqlalchemy da essere salvati sul db
		@param sqlalchemy.session: sessione attiva col db
		@return: (sqlachemy.profilo_std,[sqlalchemy.rel_profilo_funzioni])
		"""
		p=profilo_std()
		p.amministratore=self.isAdministrator()
		p.nome_profilo=self.name
		p.azienda_id=self.company
		session.add(p)
		session.flush()
		session.refresh(p)
		l=lambda x:0 if x is None else x
		Id=l(session.query(func.max(profilo_std.profilo_id)).first()[0])+1
		fList=[]
		if not p.amministratore:#il profilo e' un dict
			for i in self.profile.iterkeys():
				r=rel_profilo_funzioni()
				r.profilo_id=Id
				r.funzione_id=i
				fList.append(r)
		return (p,fList)
	...
	def addElement(self,k,v):
		"""aggiunge una funzione abilitata al profilo
		@param object:id della funzione
		@param object: in genere Function
		"""
		if type(self.profile)==type(4):
			pass#e' un profilo amministratore non ha bisogno di nulla
		else:
			self.profile[k]=v
